...
// globals
// Global Variables
std::vector<class DetectStats> g_detect_stats;
int g_video_index;
...
struct DecodeThread : public MyThread {
  ...
  virtual int run() override {
    ...
    // zak -- check to see whether we should stop
    if ((video_ended) || (dummy_frames == 1200)) {
      return -1;
    }
#else
     auto &cap = *video_stream_.get();
    ...
    if (queue_overrun == 0) {
      if (queue_->size() == DECODE_QUEUE_SIZE) {
	queue_overrun = 1;
	std::cout << "DecodeThread: queue_overrun\n";
      }
    }
    LOG_IF(INFO, ENV_PARAM(DEBUG_DEMO))
  ...
  virtual int run2() { //  override {
    static std::vector<cv::Mat> images;
    images.reserve(IMAGE_BATCH_SIZE);
    ...
    for (int i = 0; i < IMAGE_BATCH_SIZE; i++) {
      ...
      auto video_ended = images[i].empty();
      // zak -- check to see whether we should stop
      if (video_ended) {
	return -1;
      }
      while (!queue_->push(FrameInfo{channel_id_, ++frame_id_, images[i]},
			   std::chrono::milliseconds(500))) {
	if (is_stopped()) {
	  return -1;
	}
...
struct ClassificaitonDecodeThread : public MyThread {
  ...
  virtual std::string name() override {
    return std::string{"ClassificationDecodeThread-"} +
...
struct DpuThread : public MyThread {
  DpuThread(std::unique_ptr<Filter> &&filter, queue_t *queue_in,
      ...
      : MyThread{},
        ...
        suffix_{suffix} {
	  LOG_IF(INFO, ENV_PARAM(DEBUG_DEMO)) << "INIT DPU" << suffix;
	}
  virtual ~DpuThread() {}
  ...
  virtual int run() override {
    ...
    if (!points_.empty()) {
      ...
      if (frame.mat.cols > 200)
        //cv::putText(frame.mat, std::string("FPS: ") + std::to_string(fps,2),
	cv::putText(frame.mat, string_format("FPS: %0.2f; FrameID: %d", fps, frame_id),
...
inline void parse_opt(int argc, char *argv[], int start_pos = 1) {
  ...
  // Zak -- 2022-01-29 -- check to see whether path is a directory
  if (optind == argc -1) {
    std::string path = std::string(argv[optind]);
    if (std::filesystem::is_directory(path)) {
      std::cout << "Found a Directory - adding mp4 files found at:" + path +"\n";
      for (const auto & file : std::filesystem::directory_iterator(path)) {
	std::string mp4_file_name = file.path();
	std::string mp4_suffix = ".MP4";
	if (!hasEnding(mp4_file_name, mp4_suffix)) {
	  // skip non-mp4 files
	  continue;
	}
	std::cout << "  adding file: "+ mp4_file_name + "\n";
	g_avi_file.push_back(mp4_file_name);
      }
    } else {
      g_avi_file.push_back(std::string(argv[optind]));
    }
  } else {
...
int main_for_video_demo(int argc, char *argv[],
  ...
  g_video_index = 0;
  g_detect_stats.reserve(size(g_avi_file));

  for (std::string mp4_filename : g_avi_file) {
#if USE_DRM
    cv::VideoCapture video_cap(mp4_filename);
    std::string file_name(mp4_filename);
    bool is_camera =
    ...
    auto channel_id = 0;
    auto decode_queue = std::unique_ptr<queue_t>{new queue_t{DECODE_QUEUE_SIZE}};
    auto decode_thread = std::unique_ptr<DecodeThread>(
        new DecodeThread{channel_id, mp4_filename, decode_queue.get()});
    ...
    std::cout << "g_num_of_threads[0] = " << g_num_of_threads[0] << "\n";
    ...
    // start everything
    g_detect_stats[g_video_index].init();

    MyThread::start_all();
    gui_thread->wait();
    g_detect_stats[g_video_index].print_summary();
    MyThread::stop_all();
    MyThread::wait_all();
  }
